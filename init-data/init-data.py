...
def fetch_and_merge(api_url, raw_file_path):
    try:
...
def fetch_region(region):
    ...
    while date_time < date_time.now() - timedelta(days=182.5):
        future_dt = date_time + timedelta(days=182.5)
        fetch_and_merge(region['url'] + api_params + f'&casOd={date_time.isoformat()}&casDo={future_dt.isoformat()}',
